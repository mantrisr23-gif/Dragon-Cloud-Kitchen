...
const StrokeText: React.FC<StrokeTextProps> = ({
  text,
  strokeColor = '#A66A25', // Using your Bronze accent
  fillColor = '#F1EDE5',   // Using your Ivory
  strokeWidth = 1.5,
  drawDuration = 1.6,
  fillDelay = 0.2,
  stagger = 0.08,
  ...
  useEffect(() => {
    ...
    const targets = [...(strokes as any[]), ...(fills as any[]), wipe].filter(Boolean);
    ...
    const build = () => {
      ...
      if (useWipe && wipe) {
        ...
        tl.to(fills, { opacity: 1, duration: fillDuration, ease: 'power2.out', stagger: staggerConfig as any }, drawDuration + fillDelay);
      }
      return tl;
    };
